...
class MobileAuthService:
    ...
    def signup(self, email: str, password: str, full_name: str,
               ...
               invite_code: Optional[str] = None) -> User:
        """Create an account.

        Without an invite code the account is PENDING (is_active=False): it
        cannot log in until an admin activates it and attaches a home — that
        keeps who-becomes-a-customer under admin control and makes
        unverified-email registrations harmless.

        With a valid invite code (generated by an admin for a specific home)
        the code IS the approval: the account is created ACTIVE and attached
        to that home in the same step. The code is validated BEFORE the user
        row is written, so a bad code creates nothing.

        Raises:
            ValueError: invalid email/name/password, or email already registered.
            InvalidInviteError: invite code unknown/expired/exhausted/revoked.
            RateLimitedError: too many signups (or bad codes) from this IP.
        """
        ...
        invite_key = f"invite:{client_ip}" if client_ip else ""
        if self._throttle.is_locked(ip_key, invite_key):
            raise RateLimitedError()
        if ip_key:
        ...
        invite = None
        code = (invite_code or "").strip() if isinstance(invite_code, str) else ""
        if code:
            if self._invites is None:
                raise ValueError("invite codes are not enabled on this server")
            try:
                invite = self._invites.validate(code)
            except InvalidInviteError:
                # Bad codes count towards a separate per-IP lockout so the
                # keyspace cannot be brute-forced through sign-up.
                self._throttle.record_failure(invite_key)
                raise
        ...
        if invite is not None:
            self._invites.redeem(invite.code, created.user_id)
            self._throttle.record_success(invite_key)
            logger.info(f"signup via invite user={created.user_id} email={email} "
                        f"home={invite.home_id}")
        else:
            logger.info(f"signup pending user={created.user_id} email={email}")
        return created

    def redeem_invite(self, user_id: str, code: str,
                      client_ip: Optional[str] = None) -> Optional[User]:
        """Attach an already logged-in user to another home via invite code.

        Returns the user (so the caller can rebuild the bootstrap payload),
        or None when the user is missing/inactive.

        Raises:
            InvalidInviteError, RateLimitedError, ValueError (invites disabled)
        """
        if self._invites is None:
            raise ValueError("invite codes are not enabled on this server")
        keys = (f"invite:{user_id}", f"invite:{client_ip}" if client_ip else "")
        if self._throttle.is_locked(*keys):
            raise RateLimitedError()
        user = self._users.get_by_id(user_id)
        if user is None or not user.is_active:
            return None
        try:
            self._invites.redeem(code, user_id)
        except InvalidInviteError:
